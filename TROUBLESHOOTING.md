...
系统已优化：
- **比分更新**: 每10秒自动刷新（原30秒）
- **价格更新**: WebSocket实时推送 + REST API轮询备份
...
**WebSocket价格日志**（实时）：
```
[WebSocket] 💰 Lakers vs Warriors - Token 1234567890... price: 62.3¢
[WebSocket] ✅ Lakers 62.3¢ vs Warriors 37.7¢
```
...
3. **检查WebSocket状态**：
   ...
   控制台应该显示：
   [WebSocket] ✓ Connected
   [WebSocket] Subscribing to X tokens
   ```
4. **尝试禁用WebSocket**：
   ```bash
   # .env文件
