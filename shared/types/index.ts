...
export interface OddsSource {
  bookmaker: string; // 博彩公司名称 (如 "Pinnacle", "Bet365")
  homeOdds: number; // 主队赔率 (Decimal 格式，如 1.85)
  awayOdds: number; // 客队赔率
  homeProb: number; // 转换后的主队胜率 (0-1)
  awayProb: number; // 转换后的客队胜率
  lastUpdate: number; // 最后更新时间
}

/** 综合赔率数据 */
export interface BettingOdds {
  sources: OddsSource[]; // 各博彩公司的实时赔率
  averageHomeProb: number; // 平均主队胜率
  averageAwayProb: number; // 平均客队胜率
  bestHomeProb: number; // 最高的（对套利最有利的，即赔率最低的？）主队胜率 -> 这里要小心，我们是和 Poly 价格比。如果博彩公司赔率显示胜率更高，说明 Poly 更有套利价值。
}
